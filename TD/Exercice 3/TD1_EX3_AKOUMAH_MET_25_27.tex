...
The integrability condition called shape-invariance originates in supersymmetric quantum mechanics [1,2]. The separable positive-definite Hamiltonian $ \hat{H_1} = \hat{A}^{\dagger} \hat{A} $ is called shape-invariant if the condition 
\begin{equation}
     \hat{A}(a_1) \hat{A}^{\dagger}(a_1) = \hat{A}^{\dagger}(a_2) \hat{A}(a_2) + R(a_2) 
\end{equation}
is satisfied [3]. In this equation $a_1$ and $a_2$ represent parameters of the Hamiltionian. The parameter $a_2$ is a function of $a_1$ and the remainder $R(a_1)$ is independent of the dynamical variables such as position and momentum. Even though not all exactly-solvable problems are shape-invariant [4], shape invariance, especially in its algebraic formulation [5-7], has prove, to be a powerful technique to study exactly-solvable systems.\\
\vspace{0,5cm}
...
\begin{equation}
    i\hbar\frac{\partial}{\partial t}|\Psi(t)\rangle = (\hat{H_0} + \hat{H}_{int}) |\Psi(t)\rangle
\end{equation}
\noindent
we can write the wavefunction as
\begin{equation}
    |\Psi(t)\rangle = exp(-i\hat{H_0}t/\hbar) |\Psi_i(t)\rangle ,
\end{equation}

\noindent
and, by substituting this into Schrödinger equation and taking into account the commutation property between $\hat{H_0}$ et $\hat{H_{int}}$ , we obtain

\begin{equation}
    i\hbar\frac{\partial}{\partial t}|\Psi_i(t)\rangle = \hat{H}_{int}|\Psi_i(t)\rangle
\end{equation}
\noindent
We introduce the evolution matrix $\hat{U}_i(t,0)$ :
\begin{equation}
...
\begin{equation}
    i\hbar\frac{\partial}{\partial t}\hat{U}_i(t,0) = \hat{H}_{int} \hat{U}_i(t,0) ,
\end{equation}
...
\begin{equation}
    i\hbar\frac{\partial^2}{\partial t^2}\hat{U}_i(t,0) = \hat{H}_{int}\frac{\partial}{\partial t}\hat{U}_i(t,0) = \frac{1}{i\hbar}\hat{H}_{int}^2(t,0)
\end{equation}
\noindent
...
\noindent
Since by initial conditions $\hat{U}_i(0,0) = \hat{I}$, , then we can write the solution of the evolution matrix differential equation (3.7) as

\begin{equation}
    \hat{U}_i(t,0) = \left [\begin{array}{cc}
...
\begin{equation}
    \hat{U}_i^{\dagger}(t,0)\hat{U}_i(t,0) = \hat{U}_i(t,0)\hat{U}_i^ {\dagger}(t,0) = \hat{I} .
\end{equation}
\end{document}
